...
## Data 
FastQ files were downloaded from [Renyi's DropBox](https://rutgers.app.box.com/s/ott485sln5bv5px5rggf0j84igigjzf5). A total of 80 files (2 per sample, pair-ended).
...
cd nrf2ubiome/fastq_nov2018/

# Download a zip folder with all files
# curl -L https://rutgers.app.box.com/s/ott485sln5bv5px5rggf0j84igigjzf5 > download.zip
# Copy the whole folder from my machine
# 16s_2018.zip
...
# NOTE: the files unzipped into a subfolder; 
# move them one folder up to fastq_nov2018 and delete the subfolder!
```
...
dt.meta <- data.table(Name = list.files(path = path,
                                        pattern = ".gz"))
dt.meta$Name <- gsub(pattern = "_L001_R1_001.fastq.gz",
                     replacement = "",
                     x = dt.meta$Name)
dt.meta$Name <- gsub(pattern = "_L001_R2_001.fastq.gz",
                     replacement = "",
                     x = dt.meta$Name)
dt.meta <- unique(dt.meta)

dt.meta$WEEK <- factor(substr(dt.meta$Name, 1, 1))
dt.meta$Cage <- factor(substr(dt.meta$Name, 2, 2))
dt.meta$Sex <- "NA"
dt.meta$Sex[dt.meta$Cage %in% c("A", "B")] <- "Male"
dt.meta$Sex[dt.meta$Cage =="C"] <- "Female"
dt.meta$Sex <- factor(dt.meta$Sex)
dt.meta$TREATMENT <- factor(substr(dt.meta$Name, 5, 5),
                       levels = c("C", 
                                  "P"),
                       labels = c("AIN93M Control",
                                  "PEITC"))
dt.meta$MouseNum <- factor(as.numeric(substr(dt.meta$Name, 3, 3)))
dt.meta$ID <- paste(dt.meta$TREATMENT,
                    dt.meta$Cage,
                    dt.meta$MouseNum,
                    sep = "_")
dt.meta$TREATMENT_WEEK <- paste(dt.meta$TREATMENT,
                           dt.meta$WEEK,
                           sep = "_")

dt.meta <- data.frame(dt.meta)
save(dt.meta,
     file = "data_nov2018/dt.meta.RData")
