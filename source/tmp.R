...
plot_taxa_summary = function(physeq, Rank, GroupBy = NULL){
  ...
  dt1[, RankFac := factor(dt1[[Rank]], 
                          levels = rev(unique(dt1[[Rank]])))]
  dt1[, ebarMax := max(c(0, min(meanRA + sdRA)),
                       na.rm = TRUE)[1],
      by = eval(Rank)]
  dt1[, ebarMin := max(c(0, min(meanRA - sdRA)),
                       na.rm = TRUE)[1],
      , by = eval(Rank)]
...
t1 <- summarize_taxa(physeq = ps,
               Rank = "Phylum",
               GroupBy = "Diet_Week")

tiff(filename = "tmp/phylum.tiff",
     height = 5,
     width = 6,
     units = 'in',
     res = 300,
     compression = "lzw+p")
plot_taxa_summary(physeq = ps,
                  Rank = "Phylum",
                  GroupBy = "Diet_Week") +
  ggtitle("Phylum")
graphics.off()

tiff(filename = "tmp/class.tiff",
     height = 5,
     width = 8,
     units = 'in',
     res = 300,
